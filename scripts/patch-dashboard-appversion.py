...
def get_cached_app_version(dep_name, version):
    """Read the previous chart's appVersion from the lint job's chart cache."""
    chart_path = Path(".cache/upstream-charts") / Path(dep_name).name
    if not (chart_path / "Chart.yaml").is_file():
        return None

    try:
        result = subprocess.run(
            ["helm", "show", "chart", str(chart_path)],
            capture_output=True,
            text=True,
            check=True,
        )
    except Exception as e:
        print(f"Warning: Failed to read cached appVersion for {dep_name}:{version} - {e}")
        return None
    ...
    for line in result.stdout.split("\n"):
        ...
        if separator and key in {"version", "appVersion"}:
            chart_metadata[key] = value.strip(STRIP_CHARS)

    if chart_metadata.get("version") != version:
        print(f"Warning: Cached chart for {dep_name} does not match previous version {version}")
        return None

    return chart_metadata.get("appVersion", "Unknown")
...
def main():
    ...
    for package_file, dep in dependencies:
        ...
        curr_app = get_cached_app_version(dep_name, current_version)
        if curr_app is None:
        ...
        if curr_app == new_app:
            app_versions[dep_name] = " *(App: Unchanged)*"
        else:
